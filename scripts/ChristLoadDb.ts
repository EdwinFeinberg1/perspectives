...
if (!ASTRA_DB_APPLICATION_TOKEN_CHRISTIANITY) throw new Error("Missing ASTRA_DB_APPLICATION_TOKEN in .env");
if (!ASTRADB_API_ENDPOINT_CHRISTIANITY) throw new Error("Missing ASTRADB_API_ENDPOINT in .env");
...
async function ensureCollection(similarity: SimilarityMetric = "dot_product") {
  try {
    await db.createCollection(ASTRADB_DB_COLLECTION_CHRISTIANITY, {
      vector: { dimension: 1536, metric: similarity },
    });
    console.log(`✅ Created collection '${ASTRADB_DB_COLLECTION_CHRISTIANITY}'`);
  } catch (err: any) {
    if (
      err?.name === "CollectionAlreadyExistsError" ||
      /already exists/i.test(err?.message)
    ) {
      console.log(`ℹ️  Collection '${ASTRADB_DB_COLLECTION_CHRISTIANITY}' already exists; continuing`);
    } else {
      throw err;
    }
  }
...
async function ingest() {
  // 1) collection handle
  const collection = await db.collection(ASTRADB_DB_COLLECTION_CHRISTIANITY);
